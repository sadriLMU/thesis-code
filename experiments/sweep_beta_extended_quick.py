"""
sweep_beta_extended_quick.py

Exploratory extension of sweep_beta_repeated.py to higher beta values, to
check whether the EA/SA fidelity gap (which narrows steadily from
beta=0.01 to beta=0.07, and is already close to zero at beta=0.15 in the
...
This is the FAST version: fewer repeats (2 instead of 5), to get a quick
read on the trend before committing to the full, statistically robust
version (sweep_beta_extended_full.py, ~90 minutes). beta=0.07 is included
as an overlap point with the existing beta sweep, as a sanity check that
this run is consistent with the already-reported results.

Configuration:
  - 6 beta values (0.07, 0.10, 0.15, 0.20, 0.25, 0.30)
  - 2 repeats per (beta, algorithm, target)
  - Same 20 target states as the main comparison

Total runs: 6 beta x 20 targets x 2 algorithms x 2 repeats = 480 runs
(roughly 25-30 minutes based on the timing of the full beta sweep)

Output:
  - results/runs/<run_id>/beta_sweep_extended_quick.csv
  - results/runs/<run_id>/beta_sweep_extended_quick_summary.csv
  - results/figures/beta_sweep_extended_quick.png

Usage:
    cd thesis-code
    python experiments/sweep_beta_extended_quick.py
"""
...
ALPHA = 1.0
BETA_VALUES = [0.07, 0.10, 0.15, 0.20, 0.25, 0.30]
...
RUN_ID = datetime.now().strftime("%Y%m%d_%H%M%S") + "_beta_sweep_extended_quick"
...
def plot_comparison(summary_rows, path):
    ...
    ax.set_title(f"Beta sweep, EXPLORATORY extension (N={N_REPEATS} repeats/target/beta)\n"
                 f"error bars = within-target std -- quick check, not final statistics")
    ax.legend()
...
if __name__ == "__main__":
    rows = run_sweep()
    save_raw_csv(rows, os.path.join(RUN_DIR, "beta_sweep_extended_quick.csv"))
    summary_rows = save_summary_csv(
        rows, os.path.join(RUN_DIR, "beta_sweep_extended_quick_summary.csv"))
    save_config(os.path.join(RUN_DIR, "config.json"))
    plot_comparison(summary_rows, os.path.join(FIGURES_DIR, "beta_sweep_extended_quick.png"))
